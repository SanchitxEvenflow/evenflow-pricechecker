...
HTTP Client using curl_cffi for instamart's BFF API.
...
instamart_BFF_URL = os.getenv("instamart_BFF_URL", "https://bff-gateway.instamart.com")

# Headers reverse-engineered from the real instamart web app (Firefox on Windows).
# The domain changed from instamartnow.com -> instamart.com.
BROWSER_HEADERS = {
    ...
    "Pragma": "no-cache",
    "Origin": "https://www.instamart.com",
    "Referer": "https://www.instamart.com/",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:151.0) Gecko/20100101 Firefox/151.0",
    "Sec-Fetch-Dest": "empty",
    "Sec-Fetch-Mode": "cors",
    "Sec-Fetch-Site": "same-site",
    "Connection": "keep-alive",
    # -- instamart-specific routing headers (required to pass CloudFront WAF) --
    "platform": "WEB",
    "app_version": "16.0.4",
    "appVersion": "16.0.4",
    "tenant": "instamart",
    ...
    "compatible_components": (
        ...
        "instamart_PASS:5,MARKETPLACE_REPLACEMENT,PLP_ON_SEARCH,"
...
def _extract_instamart_product(json_data: dict) -> dict:
    """
    Extract product details from instamart's PDP (v2/get_page) JSON response.
    ...
    try:
        ...
        print(f"[instamart] Extraction fallback triggered: {e}")
        return {
...
def fetch_instamart_data(
    ...
    """
    Fetch product data from instamart's PDP endpoint.

    Uses the v2/get_page endpoint with page_type=PDP and the product_variant_id
    query parameter, matching the real instamart web app behaviour.
    ...
    """
    product_url = f"https://www.instamart.com/pn/product/{item_id}"
    ...
    print(f"[instamart] {city}: fetching item={item_id}, store={store_id}")
    ...
    for attempt in range(max_proxy_attempts + 1):  # +1 = direct-connection fallback
        ...
        if attempt < max_proxy_attempts and proxy_manager:
            ...
            print(f"[instamart] {city}: trying direct connection (attempt {attempt + 1})")
        ...
        try:
            ...
            prod_url = (
                f"{instamart_BFF_URL}/lms/api/v2/get_page"
                f"?store_id={store_id}"
                f"&page_type=PDP"
                f"&version=v3"
                f"&latitude={lat}"
            ...
            print(f"[instamart] {city}: GET {prod_url[:120]}... (attempt {attempt + 1}/{max_proxy_attempts + 1})")
            response = session.get(prod_url, headers=headers, timeout=15)

            print(f"[instamart] {city}: HTTP {response.status_code} ({len(response.content)} bytes)")
            ...
            if response.status_code != 200:
                logger.error("instamart returned status %d for item_id=%s city=%s", response.status_code, item_id, city)
                print(f"[instamart] {city}: Response snippet: {response.text[:300]}")
                if proxy_manager:
                    proxy_manager.report_failure(proxy)
                if attempt < max_proxy_attempts:
                    continue
            ...
            try:
                ...
                print(f"[instamart] {city}: invalid JSON: {response.text[:300]}")
                raise e

            # Extract product data
            product = _extract_instamart_product(json_data)
            print(f"[instamart] {city}: OK -- {product['title']} = Rs.{product['price']}")
            ...
            print(f"[instamart] {city}: NETWORK ERROR (attempt {attempt + 1}): {e}")
            if proxy_manager:
                proxy_manager.report_failure(proxy)
            if attempt < max_proxy_attempts:
                continue
            ...
            print(f"[instamart] {city}: UNEXPECTED ERROR (attempt {attempt + 1}): {e}")
            if proxy_manager:
                proxy_manager.report_failure(proxy)
            if attempt < max_proxy_attempts:
                continue
